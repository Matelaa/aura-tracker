...
public final class GlbWriter {
    ...
    public static byte[] write(@NonNull MeshData mesh, @NonNull TextureLookup textures,
        ...
        asset.addProperty("generator", "AuraTracker");
    ...
    private static JsonObject buildMaterial(MeshData.Primitive primitive,
        ...
        final JsonObject auraTracker = new JsonObject();
        if (hasImage && texture.isAnimated()) {
            auraTracker.addProperty("textureId", texture.getId());
            auraTracker.addProperty("scrollU", texture.getScrollU());
            auraTracker.addProperty("scrollV", texture.getScrollV());
        }
        if (auraTracker.size() > 0) {
            final JsonObject extras = new JsonObject();
            extras.add("auraTracker", auraTracker);
